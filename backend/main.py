...
async def fetch_matches():
    ...
    async with aiohttp.ClientSession() as session:
        response = await fetch(session, offers_url, payload)
        ...
        tasks = [process_match(session, match) for match in data]
        return await asyncio.gather(*tasks)

@app.route('/')
def index():
    return "It's alive on http://localhost:{}".format(port)

@app.route('/mozzart', methods=['GET'])
async def mozzart():
